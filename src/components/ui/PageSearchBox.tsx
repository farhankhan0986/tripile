...
import { Search, Calendar as CalendarIcon, ChevronDown, Users, ArrowLeftRight, BedDouble } from "lucide-react";
    ...
    <div className={className ?? "w-full max-w-4xl"}>
      ...
      >
        ...
        <div className="flex flex-wrap items-center justify-between gap-[8px] px-[16px] pt-[14px] pb-[2px]">
          <button
            type="button"
            onClick={() => setActiveTab(activeTab === "flights" ? "hotels" : "flights")}
          ...
          {activeTab === "flights" && (
            <div className="flex items-center gap-[6px] flex-wrap">
              <div className="flex rounded-full overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.16)" }}>
...
>
              ...
              <div className="flex items-center gap-[6px] px-[10px] py-[8px] rounded-full" style={{ border: "1px solid rgba(255,255,255,0.16)" }}>
                <Users size={11} style={{ color: "rgba(255,255,255,0.55)" }} />
                <button type="button" onClick={() => setPassengers((p) => Math.max(1, p - 1))} className="font-body text-[14px] font-bold leading-none w-[16px] text-center hover:opacity-80 transition-opacity" style={{ color: "rgba(255,255,255,0.65)" }}>−</button>
                <span className="font-body text-[12px] font-medium w-[10px] text-center" style={{ color: "#fff" }}>{passengers}</span>
                <button type="button" onClick={() => setPassengers((p) => Math.min(9, p + 1))} className="font-body text-[14px] font-bold leading-none w-[16px] text-center hover:opacity-80 transition-opacity" style={{ color: "rgba(255,255,255,0.65)" }}>+</button>
              </div>
              ...
              <div ref={cabinRef} className="relative">
                <button
                  ...
                  style={{ border: "1px solid rgba(255,255,255,0.16)", color: "rgba(255,255,255,0.75)", background: cabinOpen ? "rgba(255,255,255,0.10)" : "transparent" }}
                >
                  {cabinLabel} <ChevronDown size={11} style={{ opacity: 0.6 }} />
                </button>
                {cabinOpen && (
                  <div className="absolute right-0 top-[calc(100%+6px)] z-[9999] rounded-[12px] overflow-hidden py-[4px]" style={{ background: "rgba(18,7,5,0.97)", backdropFilter: "blur(20px)", border: "1px solid rgba(255,255,255,0.10)", boxShadow: "0 12px 40px rgba(0,0,0,0.55)", minWidth: "160px" }}>
                    {CABIN_OPTIONS.map((o) => (
                      <button key={o.value} type="button" onClick={() => { setCabinClass(o.value); setCabinOpen(false); }} className="w-full text-left px-[14px] py-[9px] font-body text-[13px] transition-colors duration-150" style={{ color: cabinClass === o.value ? "#C9A84C" : "rgba(255,255,255,0.75)", background: cabinClass === o.value ? "rgba(201,168,76,0.10)" : "transparent", fontWeight: cabinClass === o.value ? 600 : 400 }}>
                        {o.label}
                      </button>
                    ))}
                  </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch">
          {activeTab === "flights" ? (
            <>
              <AirportInput label="From" placeholder="City or airport" iata={fromIata} display={fromDisplay} onChange={(iata, display) => { setFromIata(iata); setFromDisplay(display); }} />
              {/* <Separator /> */}
              <div className="hidden sm:flex items-center justify-center w-0 overflow-visible z-10">
                <button type="button" title="Swap" onClick={() => { setFromIata(toIata); setToIata(fromIata); setFromDisplay(toDisplay); setToDisplay(fromDisplay); }} className="w-[28px] h-[28px] rounded-full flex items-center justify-center shrink-0 transition-all duration-200 hover:rotate-180" style={{ background: "rgba(255,255,255,0.14)", border: "1px solid rgba(255,255,255,0.22)" }}>
                  <ArrowLeftRight size={12} style={{ color: "#fff" }} />
                </button>
              </div>
              <Separator />
              <AirportInput label="To" placeholder="City or airport" iata={toIata} display={toDisplay} onChange={(iata, display) => { setToIata(iata); setToDisplay(display); }} />
              <Separator />
              <DateField label="Departure" value={departure} onChange={setDeparture} />
              {tripType === "return" && (
                <><Separator /><DateField label="Return" value={returnDate} onChange={setReturnDate} minDate={departure ?? new Date()} /></>
              )}
            </>
          ) : (
            <>
          ...
          <div className="flex items-center justify-center py-[12px] sm:py-0 sm:pr-[14px] sm:pl-[8px]">
            <button
              ...
              style={{ background: "linear-gradient(135deg, #5C1828, #8B2A3F)", boxShadow: "0 4px 16px rgba(92,24,40,0.45)" }}
            >
